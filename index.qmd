...
* **Modularity**: interfaces with and enhances base R / popular packages
...
```
::: {.r-stack}
<img alt="Steam train" src="images/steam_train.jpeg" height="300" class="fragment" />

<img alt="Toy train" src="images/toy_train.webp" height="300" class="fragment" />
:::
:::
::: {.column width="50%"}
```r
...
#> [1] 1
```
<img alt="Shinkansen" src="images/nozomi.webp" height="300" />
...
#> [1] 0.95
```
<img alt="correct" src="images/correct.png" width="80" />
...
```r
tasks <- c(1, 1, 4, 4, 1, 1, 1, 1)
```
. . .

```r
system.time(parallel::parLapply(X = tasks, fun = Sys.sleep))
#>    user  system elapsed 
#>   0.008   0.073   8.075
```
. . .
...
```r
system.time(parallel::parLapplyLB(X = tasks, fun = Sys.sleep))
#>    user  system elapsed 
#>   0.026   0.063   5.084
```
...
```r
system.time(mirai::mirai_map(tasks, Sys.sleep)[])
#>    user  system elapsed 
#>   0.005   0.002   4.006
```
